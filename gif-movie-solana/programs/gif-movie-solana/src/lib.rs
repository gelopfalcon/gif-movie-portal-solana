...
declare_id!("rSwbCDAGitQBR8BMAyy2jDpsFMZhGSN5kKaZNhEXMuW");

#[program]
pub mod gif_movie_solana {
    use super::*;
...
pub struct Initialize<'info> {
    #[account(
        init, seeds = [b"gif_account", user.key().as_ref(), gif_url.as_bytes()], bump,
        payer = user, space = 8+32+gif_url.as_bytes().len()+4
    )]
